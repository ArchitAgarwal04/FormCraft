...
export default function Hero() {
  ...
  className={cn(
    soria.className,
    "relative text-center font-extrabold text-4xl lg:text-6xl xl:text-8xl mt-8 tracking-wide capitalize"
  )}
>
  {hero.heading.split("\n").map((line, index) => (
    <span key={index} className="block mb-6 animate-fadeIn">
      {reactStringReplace(line, /\*\*(.*)\*\*/g, (match, i) => (
        <span
          key={i}
          className="relative text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 neon-glow"
        >
          {match}
        </span>
      ))}
